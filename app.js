...
function renderPickChips(match, selectedPick, canPick) {
  ...
  // 🔥 CHANGED: Removed the "draw" object from this array
  const chips = [
  ...
  return chips.map(({ pick, label }) => {
    ...
    if (selectedPick === pick) {
      if (match.status === "finished") {
        // If the match ended in a draw, everyone who picked a team wins a point anyway
        if (winner === "draw") {
          classes += " correct";
        } else {
...
async function loadPickHistory() {
  ...
  el.innerHTML = picks.map(p => {
    ...
    if (match.status === "finished") {
      // 🔥 CHANGED: Scoring display metrics rules tracking
      if (winner === "draw") {
    ...
    const pickedTeamLabel = p.pick === "home" ? match.homeTeam : match.awayTeam;

    return `
      <div class="history-item">
        <span class="history-match">${match.homeTeam} vs ${match.awayTeam}</span>
        <span class="history-pick">Picked: ${pickedTeamLabel}</span>
        ${resultHtml}
      </div>
    `;
...
async function loadGroupSummary(matchId, el) {
  ...
  // 🔥 CHANGED: Visual layout strings now only compute Home vs Away metrics
  el.innerHTML = `
